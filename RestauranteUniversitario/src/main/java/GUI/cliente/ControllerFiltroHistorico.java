...
public class ControllerFiltroHistorico {
    ...
    void bttnFiltrarOn(ActionEvent event) {
        ...
        ScreenManager.getInstance().getControllerHistoricoCompras().limparItens();
        ...
        if (registroCompras.isEmpty())
            ...
            ScreenManager.getInstance().getControllerHistoricoCompras().configurarTv(registroCompras);
    ...
    private List<RegistroCompra> filtrar() {
        ...
        if (validarCampos()) {
            registrosFiltrados = Fachada.getInstance().obterRegistrosDoUsuario(Fachada.getInstance().getUsuarioLogado());
        }
        else {
            try {
                ...
                if (cbPagamento.getValue() == null) {
                    modelo = new RegistroCompra(new ArrayList<>(0), Fachada.getInstance().getUsuarioLogado().getLogin(),
    ...
    private boolean validarCampos() {
        return tfAlmoco.getText().isBlank() && tfJanta.getText().isBlank() && tfVendedor.getText().isBlank() &&
                cbPagamento.getValue().isBlank() && tfCodigo.getText().isBlank() && dtpData.getValue() == null
                && tfJanta.getText().equals("0") && tfAlmoco.getText().equals("0");
    }
